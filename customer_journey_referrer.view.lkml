view: customer_journey_referrer {
  derived_table: {
    sql: with x as (
      ...
      select avg(num_sessions) avg_num_sess,avg(avg_views_sess) avg_pageviews_per_sess, a.total_sessions,   xff.referrer from xff
      left join (select count(session_id) total_sessions, referrer from x group by referrer having count(session_id) > 50) a
      on xff.referrer = a.referrer
      where total_sessions > 50 and xff.referrer not like '%purple.com%'
  ...
  set: detail {
    fields: [avg_num_sess, avg_pageviews_per_sess, total_sessions, referrer]
  }
}
